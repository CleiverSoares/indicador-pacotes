  ...
  <nav>
    ...
    <div class="card-resultado-pai">
      <section class="card-resultados" id="resultado">
        ...
        <div v-if="selectedProductsCount > 1">
...
export default {
  setup() {
    ...
    function isRangeProdutoSelecionado(produto) {
      return produto.toLowerCase() === 'premium';
    }
    ...
    let selectedProductsCount = ref(0)
    watch(() => produtosSelecionados, () => {
      const selectedProducts = Object.entries(produtosSelecionados).filter(([key, value]) => value.selecionado);
      selectedProductsCount.value = selectedProducts.length;


      let comboEncontrado = '';
      for (const [nomeCombo, produtosCombo] of Object.entries(combos)) {
        const allComboProductsSelected = produtosCombo.every(produto => selectedProducts.find(([key]) => key === produto));
        const noAdditionalProductsSelected = selectedProducts.every(([key]) => produtosCombo.includes(key));
        if (allComboProductsSelected && noAdditionalProductsSelected) {
    ...
    function gerarURLCompra() {
      ...
      const selectedProductsIds = encodeURIComponent(idsSelecionados.join(','));
      return `https://www.google.com?produtos=${selectedProductsIds}`;
    }
    ...
    function gerarURLIFrame() {
      const idsSelecionados = Object.entries(produtosSelecionados)
        ...
        .map(([key, value]) => value.id);
      const selectedProductsIds = encodeURIComponent(idsSelecionados.join(','));
      return `https://app-mkt-leads.alterdatasoftware.com.br/formulario/contato/petshop?r=site-alterdata-empresarial&produtos=${selectedProductsIds}`;
    }

    return { produtosSelecionados, valorRange, selecionarProduto, isRangeProdutoSelecionado, gerarURLCompra, gerarURLIFrame, comboSelecionado, botaoComprarHabilitado, selectedProductsCount };
  }
}
